...
BEGIN
    ...
    BEGIN
        -- Nếu chưa có dữ liệu ngày đó thì insert
        IF NOT EXISTS (
            SELECT 1 FROM BuocChanNgay
            WHERE idTaiKhoan = @UserId
              AND Ngay = @CurrentDate
        )
        BEGIN
            DECLARE @SoBuoc INT =
                3000 + ABS(CHECKSUM(NEWID())) % 12000; -- 3000–15000

            DECLARE @Kcal FLOAT = ROUND(@SoBuoc * 0.04, 1);
            DECLARE @QuangDuong FLOAT = ROUND(@SoBuoc * 0.00075, 2);
            DECLARE @ThoiGianGiay INT =
                1800 + ABS(CHECKSUM(NEWID())) % 3600; -- 30–90 phút
