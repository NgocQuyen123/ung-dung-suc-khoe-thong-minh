...
public class PaceAdapter extends RecyclerView.Adapter<PaceAdapter.VH>{
    private List<TocDoCanNang> items;
    private String selectedId = null;
    private final OnItemClick listener;
    private final boolean isLosingWeight;


    public interface OnItemClick { void onClick(TocDoCanNang item); }

    public String getSelectedId() {
        return selectedId;
    }
    public PaceAdapter(List<TocDoCanNang> items, OnItemClick listener, boolean isLosingWeight) {
        this.items = items;
    ...
    public VH onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_pace_rate, parent, false);
        return new VH(v);
    }

    @Override
    public void onBindViewHolder(@NonNull VH holder, int position) {
        TocDoCanNang t = items.get(position);
