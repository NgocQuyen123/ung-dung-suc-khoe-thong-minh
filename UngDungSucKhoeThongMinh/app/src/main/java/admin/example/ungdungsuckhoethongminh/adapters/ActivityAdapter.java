...
public class ActivityAdapter extends RecyclerView.Adapter<ActivityAdapter.VH> {
    private List<MucDoHoatDong> items;
    private int selectedId = -1;
    private final OnItemClick listener;

    public interface OnItemClick { void onClick(MucDoHoatDong item); }

    public ActivityAdapter(List<MucDoHoatDong> items, OnItemClick listener) {
        this.items = items;
    ...
    public VH onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_activity_level, parent, false);
        return new VH(v);
    }

    @Override
    public void onBindViewHolder(@NonNull VH holder, int position) {
        MucDoHoatDong currentItem = items.get(position);
